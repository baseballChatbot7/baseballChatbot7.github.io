...
var documents = [{
    ...
    "body": "2021/06/26 - 2021. 06. 26 회의록 김남혁님     NER, MRC 경량화   MRC 학습만 다시 하면 좋을 듯하다. –&gt; 야구 데이터를 가지고 학습을 해야할 것 같다   전체적인 파이프라인 정리해놓음 -&gt; NER 추가해주면 될꺼 같다.    사람이 쓸정도로 만드려면 시간이 오래 걸릴꺼같다.    실시간 데이터 고민해야한다.    앞으로 2주 정도…카카오 톡에서 돌아가게 하면 좋을것같은데   살아 있는 동안 우리 서버에서 해도 되고 GCP를 써서 해도 될듯하다.     할 일     김남혁님         파이프 라인에 영재님 NER추가 해서 백엔드에 주기     나무위키 데이터 문서 수를 줄이는 것          종빈님 &amp; 서일님         야구 전용 MRC데이터 생성     convolution을 빼고 학습할 예정          민정님 &amp; 병민님카카오 답변 지연에 대한 이슈를 정리   "
    }, {
    "id": 10,
